...
3. 区分四类：`官方模板`、`手动补充模板`、`自建复用工作流`、`项目工作流`。官方母版与运行镜像各一份是有意同步，不是无意义重复。
4. 使用 `scripts/audit_workflows.py` 生成审计报告；任何删除或移动前先做 dry-run 和引用搜索。
5. 安全归档优先于删除：目标必须是精确路径，归档清单记录原路径、目标路径、哈希、原因和替代项。被测试、脚本或项目引用的文件不能直接删除。
6. 工作区是编辑母版，ComfyUI 用户目录是运行镜像；同步后必须回读 SHA-256。不得把用户目录里的临时自动保存文件当作正式版本。

推荐目录和命名见 `references/workflow-organization.md`。文件名应表达“输入 → 核心能力 → 输出”，例如：
...
不要为了“整齐”删除官方模板、模型、输出或项目实验资产；只有审计证明无引用、无学习价值且有可恢复归档时才删除。
