...
- WSL 受管发布按 `${CODEX_WORK_ROOT:-$HOME/Documents/work}/host-baseline/skills-management.md` 和 `scripts/publish_wsl_skills.py --skill <name>` 执行；Windows 按目标机现行 CC Switch 流程。两侧分别验收，不用一侧成功替代另一侧。
...
- 平台声明统一维护于 platforms.json，区分执行平台、远程载荷平台和业务依赖。新增技能同步声明；发布器必须在写入前核对真实系统和受管名单。该清单不是 Codex 原生过滤字段，不因声明支持就声称实机验收。公共目录由 host_platform.py 读取主机配置，个人 CODEX_HOME 仍按进程环境解析。
