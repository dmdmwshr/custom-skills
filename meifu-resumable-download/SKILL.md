...
description: 仅在下载必须经代理且文件大于等于 2 GiB 时，通过 Meifu 分块缓存稳定下载、断点续传、校验和原子落盘。可直连的下载走本机直连；必须经代理但小于 2 GiB 的下载走 CN2 直连；ComfyUI 仅在满足 Meifu 分流条件后调用本 skill。
---
...
先判断来源是否可直连、是否必须经代理，再判断文件大小；不要仅因文件是模型、数据集或安装包就使用 Meifu。

1. 来源可直连时，任意大小都走本机直连，不经过 CN2 或 Meifu。
2. 来源必须经代理但文件小于 2 GiB 时，走 CN2 直连，不使用 Meifu。
3. 只有来源必须经代理且文件大于等于 2 GiB 时，才使用本 skill 的 Meifu 后台单工作队列。
4. 是否必须经代理或文件大小无法确认时，先做只读探测；没有同时满足第 3 项前，不得默认转入 Meifu。

这里的“本机直连下载”指直接从来源站获取文件；“CN2 直连下载”指本机通过现有 CN2 网络链路直接从来源站获取文件。两者都不经过 Meifu 中转。是否下载、保存位置、校验和以及不自动执行/导入的边界仍须遵守当前用户授权。
...
只有“必须经代理且大于等于 2 GiB”的公共直链，才使用 `scripts/meifu_download_queue.py`：
...
1. 确认来源是否必须经代理、文件大小、输出位置和官方 SHA-256。ComfyUI 模型还须先确认类别、版本、许可证和最终目录。
2. 来源可直连：走本机直连，不调用本 skill；来源必须经代理但文件小于 2 GiB：走 CN2 直连，不调用本 skill。两者都不写入 Meifu 队列。
3. 只有同时满足“必须经代理”且“文件大于等于 2 GiB”时，才用不带 `--execute` 的单文件命令做 Meifu 脱敏来源/输出规则预检；需要验证 Range、远端和本机空间时使用 `--probe-only`。
4. 当前消息已明确提供或批准来源链接、输出位置和下载动作时，视为直接批准；否则在只读预检后一次性确认精确链接、目标和影响。批准范围内的候选直接逐项 `enqueue --requested-by <稳定任务标识>`，再一次 `start` 请求 Windows 后台任务，不重复确认。
5. 只用 `status`、`list`、`audit` 轮询。运行中重复请求会在创建子进程前返回“已有传输正在进行”。需要增删、重排或恢复条目时，先按页 `list` 或按精确条目编号查看，再使用精确条目编号和变更原因。
6. 完成后核对输出大小、SHA-256 和“来源已验证/仅计算哈希”状态；不自动执行、安装或导入下载文件。
...
- ComfyUI 先按本文件的“下载路由规则”分流：可直连的候选走本机直连；必须经代理但小于 2 GiB 的候选走 CN2 直连；只有必须经代理且大于等于 2 GiB 的已批准候选，才通过 `delegate_model_downloads.py` 写入本队列。
