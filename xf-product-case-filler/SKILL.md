...
description: 从消防产品案卷截图或截图 OCR 结果中抽取案件信息，直接更新“产品案卷数据.xlsx”对应大队工作表，支持标称生产者、初检/复检去重、按项目编号更新、同步各大队模板列结构、行政案件默认行案并标黄复核，并按检查日期、单位名称原地重命名已处理截图。用于用户提到产品案卷、消防产品检查案卷截图、案卷清单、产品监督检查记录表、月度任务表填报、按项目编号去重更新、复检不覆盖初检、罚字/行政案件/行案刑案或截图归档命名时。
---
...
5. 读取 `references/field-rules.md`，按字段规则整理 JSON。关键点：
   ...
   - 截图标题、文书号或附件中出现明显 `罚字` 时，`case_type` 默认写 `行案`，同时设置 `case_type_review: true` 让脚本把单元格标黄；能明确是刑案时才写 `刑案`。
