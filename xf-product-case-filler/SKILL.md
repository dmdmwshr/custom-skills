...
description: 从消防产品案卷截图或截图 OCR 结果中抽取案件信息，直接更新“产品案卷数据.xlsx”对应大队工作表，支持标称生产者、初检/复检去重、按项目编号更新、同步各大队模板列结构，并按检查日期、单位名称原地重命名已处理截图。用于用户提到产品案卷、消防产品检查案卷截图、案卷清单、产品监督检查记录表、月度任务表填报、按项目编号去重更新、复检不覆盖初检或截图归档命名时。
---
...
2. 如果模板工作表新增列或调整列位，先同步其他大队工作表：
   ...
   ```
3. 按数字重复命名分组截图，例如 `1.png` 与 `11.png` 属于同一案。不要仅按文件名判断截图类型，要根据标题和内容识别 `案卷清单`、`检查记录表` 或 `其他附件`。同一案卷号出现初检和复检时，只抽取初检，复检图片只作为同案附件或备注。
