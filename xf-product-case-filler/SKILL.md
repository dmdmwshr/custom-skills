...
description: 从消防产品案卷截图或截图 OCR 结果中抽取案件信息，直接更新“产品案卷数据.xlsx”对应大队工作表，支持标称生产者、初检/复检去重、按项目编号更新、同步各大队模板列结构、行政案件默认行案并标黄复核，并按检查日期、单位名称原地重命名已处理截图。用于用户提到产品案卷、消防产品检查案卷截图、案卷清单、产品监督检查记录表、月度任务表填报、按项目编号去重更新、复检不覆盖初检、[罚]/罚字/行政案件/行案刑案或截图归档命名时。
---
...
2. 如果模板工作表新增列或调整列位，先同步其他大队工作表：
   ...
   ```
3. 按重复命名分组截图：数字组如 `1.png`、`11.png`、`111.png` 属于同一案；字母组如 `a.png`、`aa.png`、`aaa.png` 属于同一案。不要仅按文件名判断截图类型，要根据标题和内容识别 `案卷清单`、`检查记录表` 或 `其他附件`。同一案卷号出现初检和复检时，只抽取初检，复检图片只作为同案附件或备注。
...
- 脚本按 `--brigade` 匹配工作表，按表头定位 `项目编号` 列去重。项目编号已存在时更新同一行，不新增重复行。
- 新案件写入第一个目标字段全空的行；如果模板行已满，脚本复制末行样式追加新行并延续序号。
- 只写目标字段对应的数据列，不改无关单元格。
- `检查时间`、`是否为微型消防站或快速处置队`、`现场判定或抽样送检`、`是否合格`、`是否为行案/刑案`、`是否为网售` 只写入允许值。非法值会被留空并标红。
