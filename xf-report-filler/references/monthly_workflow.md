...
2. 目录整理：按“两层月份模型”整理知识库模板库中的 `月通报模板` 根层和 `（X-1）月巡查` 子目录；实际通报目录下生成具体月份目录，如 `26年\1、月度通报\2607_通报\6月巡查`。
3. 根层 pending：25 号前生成或刷新当月两张根层表，文件名前加 `【待补】`，表内待确认单元格红底；R02 的 B:J 为待确认区，B 列补人员要求数，I、J 列由大队按“总次数（不合格次数）”人工填报；工作动态只由目录整理识别和保留。
4. 巡查成绩生成：从产品底册、联网基础信息截图、联网统计表生成 G01-G05 成绩材料；产品监督档案内的评查日期按通报月份写，文件名和成绩数据仍按巡查月份写。
...
- `R03`：`monthly/output_R03_work_dynamic.md`
成绩月份成品：
