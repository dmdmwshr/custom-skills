...
- `template_sources.numbered_library`
- `template_sources.bulletin_skeleton`

skill 内 `resources/monthly_templates` 是快照，只用于哈希校验和兜底审计。

## 生成规则

- 正式生成优先使用外部模板。
- 外部模板和 skill 快照哈希不一致时，采用外部模板并 warning。
- 用户确认同步后，`sync_monthly_templates.py --apply` 才把外部模板复制到 skill 快照并更新 manifest。
- T10 以 `模板文件\X月通报\X月重点工作完成情况上报表（应急通信与消防科技）.xls` 为事实源，同步时再写入 `_编号模板库` 和 skill 快照。
