...
1. **月度产品与联网监测成绩登记**：整理月度通报目录、校验模板版本、生成 `（X-1）月巡查` 成绩材料、处理当月根层两张通报表待补/定稿，并识别根层工作动态文件。
...
- 交付目录保持历史通报的干净结构：只留当月材料、上月巡查源文件和成品，以及照片、联网明细、产品成绩三个正式子目录。整理脚本在实例化前迁移占位目录中的人工材料；同名同内容去重，内容冲突保留并报告，禁止只新建空目录而不迁移。已核对哈希一致的空白模板副本删除，不再创建 `_模板副本归档`。
- 日志、渲染图、视觉收据、中间数据和必要的操作备份放在业务目录以外的本机临时工作目录，不放入通报目录，也不新建隐藏杂项目录。结束前复查占位名与杂项；历史遗留过程目录只有核对来源和内容后才按本次授权清理，不能按名字盲删。Office 锁文件存在时先关闭对应文件，不移动活动锁文件。
...
- R02《消防产品监督统计表》待补范围为 B:J：B 列只补大队人员要求数；I 列网售消防产品检查、J 列维修后灭火器检查由人工按“总次数（不合格次数）”填写，不自动推算或覆盖。
- 产品底册黄色高亮问题属于私账：不进入成品，不参与扣分。
- 产品公开问题描述只写底册括号前内容；括号内、`ps` 后、字段级细节只用于内部定位。
- 联网公开描述中 CAD 类写“缺点位图”，PDF 类写“缺火灾防控图”。
- 成绩材料显示格式：产品分数固定 1 位小数；联网监测分数在个人执法统计表固定 1 位小数，在个案成绩表固定 2 位小数。
