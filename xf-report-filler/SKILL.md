...
- 外部模板事实源固定在 `resources/monthly_workflow.json` 的绝对路径；`模板文件\X月通报` 是唯一标准模板文件夹，根层放当月通报表模板，`（X-1）月巡查` 放成绩材料模板；成绩材料模板文件和标准子文件夹使用 `YYYY年（X-1）月...` 命名；skill 内 `resources/monthly_templates` 只是快照。
