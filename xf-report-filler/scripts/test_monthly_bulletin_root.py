...
class MonthlyBulletinRootTests(unittest.TestCase):
    ...
    def test_staff_count_and_pending_value(self):
        ...
        self.assertEqual(root.product_stats_pending_value(8), "【待补】（8）")
    ...
    def test_audit_product_stats_blocks_mismatch(self):
        blockers = root.audit_product_stats(
            {"江阴大队": {"row": 9, "value": "6次（12）", "count": 6}, **{brigade: {"row": 1, "value": "1次", "count": 1} for brigade in root.BRIGADE_ORDER if brigade != "江阴大队"}},
            {"江阴大队": {"unique_projects": 11}, **{brigade: {"unique_projects": 1} for brigade in root.BRIGADE_ORDER if brigade != "江阴大队"}},
        )
    ...
    def test_mark_pending_office_marks_product_timeliness_row_red(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            changed = root.mark_pending_office(path)
            wb2 = load_workbook(path)
            ws2 = wb2["1"]

            self.assertEqual(ws2.cell(15, 4).value, "【待补】")
            self.assertEqual(ws2.cell(15, 11).value, "【待补】")
            self.assertEqual(ws2.cell(15, 4).font.color.rgb, "FFFF0000")
