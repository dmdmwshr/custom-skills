...
def select_monitor_report_cases(monitor_details):
    order_index = {name: idx for idx, name in enumerate(CASE_ORDER)}
    selected = []
    used = set()
    for detail in sorted(
        monitor_details,
        key=lambda d: (d["score"], order_index.get(d["short"], 999), d["index"]),
    ):
...
def build_report_sections(product_records, monitor_details):
    product_sentences = []
    for item in product_records:
        if item["no_case"]:
    ...
    monitor_sentences = []
    for detail in select_monitor_report_cases(monitor_details):
...
def write_report_doc(template_path, output_path, product_records, monitor_details, force):
    ...
    product_text, monitor_text = build_report_sections(product_records, monitor_details)
...
def run(args):
            ...
            json.dumps(
                {
                    ...
                    "monitor_report_cases": select_monitor_report_cases(monitor_details),
                },
                ensure_ascii=False,
                indent=2,
            )
    ...
    personal_warnings = write_personal_stats(
        month_dir / "（模板）个人执法统计表202601.xlsx",
    ...
    write_report_doc(
        ...
        month_dir / f"{args.year}年{args.month}月通报.doc",
        product_records,
        monitor_details,
        args.force,
    )
    ...
    summary = {
        ...
        "monitor_report_cases": [
            ...
            for item in select_monitor_report_cases(monitor_details)
        ],
    }
