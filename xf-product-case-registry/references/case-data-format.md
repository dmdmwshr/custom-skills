...
- 日期快捷项固定记录为“本年”，年度起止固定记录为当年 `01-01` 至 `12-31`，并保存管辖单位、可选执法大队和精确文书类型；页面实际窗口模式、`innerWidth/innerHeight` 和底部分页/操作区可达性写入批次操作摘要，不用历史固定尺寸代替；
...
- `CaseWaterlineV1`：当前批次案卷的来源、本地、上传和飞牛状态；
- 活动 `工作区/<项目编号>/upload-state.json`：V6 任务创建、文件投影、已传文件、finalize 与 VERIFIED 里程碑。不得递归读取 `历史工作区` 冒充当前进度。
...
`ledger status` 增加逐案 `completedStages`、`nextAction`、等待原因、记录中的 OCR 完成/待办数量及本地计时。它只读现有状态，不运行识别、登录、上传或归档；文件损坏、旧上传格式或项目归属不一致时提示核对，不据此创建新任务。完成案卷只能沿水位中的明确归档代际读取本地摘要，不扫描历史目录猜测当前进度；汇总数字仍按原正式批次口径计算。
