...
所有本地案卷工作固定在 `E:\文件夹\1、工作\2、产品科技联网\1、产品监督\案卷汇总`，不得写入消防产品案卷信息登记系统代码仓库：

- `原始案卷/<项目编号>`：原案卷包、原 PDF、图片及其只读留存；
- `工作区/<项目编号>`：清点、OCR、拆分计划、`normalized/`、`case-data.json`、manifest 和 `upload-state.json`；
- `工作区/核验记录`：可选保存的核验摘要目录；CLI 不自动写入该目录；
- `资料模板`：复用的业务资料模板，不存放某一案卷的运行产物。

原始案卷和工作区必须分离。新生成的规范 PDF 只写入对应项目编号的工作区；认证配置仍固定在 `%LOCALAPPDATA%\xf-product-case-registry`，不迁移至业务工作根。
...
- `repairSiteId` 仅在产品已有明确维修点 UUID 时填写；可明确表示没有维修点时填写 `null`，不确定时省略。非空 `repairSiteId` 必须配合 `maintenance: "YES"`，不得凭名称或地址猜 UUID。
...
- 拆分计划的每个目标必须位于 `工作区/<项目编号>/normalized/` 下，且不得重复、覆盖已有文件或指向 `原始案卷/<项目编号>`。空白页不生成规范 PDF。
