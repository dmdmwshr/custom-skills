...
- 年度基线记录来源页面“本年”的实际日期范围、日期字段、任务类别、全部管辖单位和全部状态；近期增量记录执行日向前回推三个月的显式起止日期及全部文书类型，支持跨年。旧批次保留原筛选口径。页面实际窗口模式、`innerWidth/innerHeight` 和底部分页/操作区可达性写入批次操作摘要，不用历史固定尺寸代替；
...
- 上述检查阶段标记仅用于默认检查记录查询。`selectionMode=ANNUAL_RECTIFICATION_NOTICE` 的 `inspectionStages` 保持空数组，不把通知书数量/顺序解释为检查次数或初查复查；另记录实际 `documentTypePage`、`dateFieldLabel` 和 `queryEvidencePath`，现场日期含义不能由“本年”两个字推断。
- 同一 RWID 下案卷名称省略或附带已知主体类型后缀时，保留每条原始名称并按规范名分组，不覆盖原文，也不单独形成阻塞冲突；
- 当前断点、重扫次数、稳定/清单变化中/失败状态及错误摘要。
- 页码和行号只是当轮位置证据；重新加载后必须以案卷名称、文书名称/编号和创建时间重新定位，再用详情 URL 的 RWID 对账。
...
- 身份链不一致时记录 `CASE_IDENTITY_CHAIN_MISMATCH` 及 `blockingScope=CURRENT_CASE`；不得把截图、包或项目工作区改绑给其他项目。

RWID 只用于来源去重和追溯，不进入 V2 manifest。Cookie、令牌、完整 URL、原始 HTML、浏览器配置和登录凭据禁止进入该格式。

## CaseWaterlineV1 与 Excel

`CaseWaterlineV1` 按经来源证据确认的项目编号维护一条全局水位，包含来源标识集合、独立的来源日期/核对时间/本地处理时间、包摘要、系统绑定和明确归档代际。先复用已验证的 RWID 与项目绑定；只有身份缺失、来源变化或未完成断点才进入必要详情。不同 RWID 对应同一项目时合并，尚未取得项目编号的来源项保留待关联。来源登记、材料采集、系统登记、正文核验、归档完成分别计算；来源指纹未变且有效证据齐全时复用，不重复下载。旧状态主线继续用于断点兼容：

`已发现 → 详情已采集 → 案卷包就绪 → 待整理 → 待上传 → 上传中 → 已上传待飞牛核验 → 已完成`

异常状态为“需人工处理”或“采集失败”，只暂停对应案卷。状态更新应先原子写 JSON；单案处理到一个可恢复里程碑立即写 JSON，批量上传结束或向用户报告进度前运行一次 `ledger export` 生成 `案卷水位记录表.xlsx`。VERIFIED 自动归档也会重建 Excel。Excel 不是事实源，用户占用文件导致导出失败时不得回滚已成功的 JSON，关闭 Excel 后可重新生成。
...
逐案报告同时展示 `uploadProgress`（已保存接收断点的总数/已接收/缺失文件）和核验等待信息。`nasVerification` 的 `RATE_LIMITED/WAIT_TIMEOUT/NETWORK_WAIT/PENDING` 属于未完成等待，`FAILED` 表示需核查异常；有效等待头对应 `retryAfterSeconds/retryAt/observedAt`，缺失值保留未知。真实冲突、无效本地状态仍优先报异常，不能因存在等待文字而自动恢复。读取报告不会读取认证、发起网络请求或改写这些状态。

进度询问固定运行 `ledger status`，默认读取所有已知案卷。年度对齐使用 `--scope source-year --source-year <年份>`，指定批次使用 `--scope batch --batch-id <批次>`。分类子集与未完成视图均来自同一总账，不以最新批次代替累计数。开始扫描、批量上传和结束时回读相同范围；验收样本始终不计入正式水位。命令只读，不更新 JSON、Excel、上传任务或浏览器状态。

查询以详情项目编号为唯一计数键，并同时回读以下事实源：

- `BrowserCaptureV1`：页面文书记录数、唯一 RWID、每页数量、总页数、稳定轮次、异常和冲突；
- `原始案卷/案卷目录截图/<批次>`：列表截图物理文件数；
- `原始案卷/待处理案卷/<项目编号>`：正式详情截图、`SourceEvidenceV1` 和唯一 ZIP；批次 `staging` 的副本单独列出，不能重复计入正式截图；
- `CaseWaterlineV1`：所选范围案卷的五项独立状态、来源年度成员关系、待关联与覆盖缺口；
- 活动 `工作区/<项目编号>/upload-state.json`：V6 任务创建、文件投影、已传文件、finalize 与 VERIFIED 里程碑；历史完成仅沿总账中明确的归档工作区引用读取，不递归猜测归档代际。

“已下载案卷”只指已有唯一正式 ZIP 的项目数；“已上传系统”必须进一步拆成已创建任务、全部文件已传、finalize 成功、存在冲突、飞牛已核验。对外回答的“系统成功案卷数”只使用 `successfulSystemCases`：V6 状态为 `FINALIZED_UNVERIFIED` 或 `VERIFIED`，且服务端摘要同时满足 `created=true`、冲突数为 0、跳过数为 0。仅创建任务、仅上传完文件、HTTP 500 后本地仍为 `UPLOADING`，均不得称为上传成功。
...
`ocr` 必须用重复的 `--relative-path` 显式选择本案 inventory 中的 PDF/PNG/JPEG，输出位于本案 `--work-dir` 的独立 `--output-dir` 子目录。当前默认逐文件模式；`--batch` 显式启用多份批量，尚不因实测提速而自动成为默认。批量只复制所选文件为稳定别名，保留源路径与哈希映射，中文、重名和嵌套目录不混入相邻案卷。原始材料不改名、不改内容。

结构完整和结果哈希只证明产物可复用，不证明识别文字正确。批量默认启用还需同类材料的页数、表格和关键字段一致性验收；手写日期、签名及图片内文字出现差异时保留逐文件默认并报告限制。两种模式都不得把不可信 OCR 猜值覆盖详情结构化数据或完整电子文书。

批量使用现有本机 MinerU 镜像和相同 hybrid/auto/ch、表格与公式设置，按镜像 ID 固定版本，一次短生命周期容器完成全部所选文件。原生 `MINERU_API_MAX_CONCURRENT_REQUESTS=1` 使任务在同一模型下顺序提交，避免冷启动时并发提交触发内部请求超时；不发布端口、不启动常驻服务、不更新镜像。省略 `--batch` 保留既有包装脚本逐文件模式。`--timeout` 为每文件预算，批量总上限为预算乘本次未完成文件数；超过上限停止本次专属容器，不自动重试。

`ocr-result.json` 扩展为 `OcrResultV1`，保留既有 `mappings`，增加本案工作区绑定、状态、待完成列表和简短运行统计。每份结果只有结构化页数、原始页副本、Markdown、内容索引与引用图片均完整，且运行中两次快照一致或进程已结束，才原子保存其哈希清单。续跑逐项比较源哈希、路径、引擎镜像、包装入口、识别选项和输出完整性；全部命中时不启动识别。缺失、损坏、源变化、旧格式或不可信索引不能仅凭目录/Markdown 存在命中缓存。索引仍保存其他已选文件的既有映射；每次统计明确指本次显式选择范围。

中断和部分失败保留已确认文件，下一次命令只处理未确认项。没有原子成功回执的输出不得猜测接纳；旧格式可读但首次使用需重新识别。进度最多每两秒只显示已确认数量、复用数量和耗时，不打印 OCR 原文、引擎长日志或图片字节。

`ledger status` 增加逐案 `completedStages`、`nextAction`、等待原因、记录中的 OCR 完成/待办数量及本地计时。它只读现有状态，不运行识别、登录、上传或归档；文件损坏、旧上传格式或项目归属不一致时提示核对，不据此创建新任务。完成案卷只能沿水位中的明确归档代际读取本地摘要，不扫描历史目录猜测当前进度；汇总按本次选择的总账、来源年度或指定批次范围计算，并单列活动待办。

`ledger report` 默认仅返回本地事实源的 Markdown 汇总；显式 `--output` 才写到当前工作根 `工作区/核验记录` 内的新 `.md` 文件，不覆盖既有报告。报告不等同于重新访问服务器核验，也不是第二套水位。人工暂停、网络等待、操作处理和不可细分历时分列；缺失历史记录为未知，并行或嵌套计时不得相加宣称端到端历时。

可核实的本地执行耗时保存为本案或本批次 `diagnostics/timings/timing-<随机标识>.json`（`WorkflowTimingV1`）；浏览器阶段计时使用批次的 `阶段_*.json`。这些文件只含类别、起止时间、耗时、结果和范围键，不存正文、身份凭据或业务响应。查询、离线验证、dry-run、只读计划不生成计时记录；已归档或缺失的活动工作区不因补记耗时而重建，未记录阶段保持未知。
...
所有、不合格、待确认、未完成为同一总账视图。年度任务列表条数、文书数、来源标识数、去重项目数和待关联数分别表达；全部类别两轮稳定且身份缺口清零前，年度覆盖保持未完成。入口暂不可用时保留页面证据与缺口，不能当作零条。年度基线及命令见 [年度基线](annual-baseline.md) 和 [统一水位](unified-waterline.md)。
