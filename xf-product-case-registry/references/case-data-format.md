...
- `原始案卷/待处理案卷/<项目编号>`：来源详情截图、原始 ZIP 和原始案卷文件；
...
- 页面实时总数、每页条数、报告页数、各扫描轮次及其 RWID 集合摘要；
- 每页项目编号/RWID 条目、列表特征、页码和完成时间；
...
连续两轮完整集合一致才能标记稳定，最多三轮。页面数量是运行时事实，不用历史截图中的 514 等数字填充。
...
`CaseWaterlineV1` 按项目编号维护一条全局水位，至少包含单位、大队、来源 RWID、标签、包哈希、来源进度、本地整理进度、上传进度、飞牛核验、完成时间和错误摘要。状态主线为：
