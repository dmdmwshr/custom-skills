...
- 多文件整卷核验先完成全部目录字段、文件归属、SHA-256 和飞牛状态检查；只有服务器整卷文件 ID 集合与本次清单精确相等，才调用现有 `POST /api/v2/cases/:id/export-preparations` 并轮询现有准备状态，再逐份取回正文比对哈希。不创建或下载 ZIP，不新增导入步骤，不放宽缓存额度。准备回执的案卷、准备任务及文件总数必须一致；按文件数设置有界等待（90～600 秒），超时保留同一断点。服务器有本次清单范围外文件时，不批量取回整卷，仍只对已授权文件逐份处理。缓存额度不足与请求频率限流分开报告，不通过重新登录绕过额度。
