...
- 单文件上传默认总期限 210 秒、连续无进展 60 秒，支持 Ctrl+C 取消。进度输出到标准错误流，最多每 2 秒更新，包含文件名、已确认接收数量、发送字节和耗时；发送字节不代表服务端已接收。机器可读结果仍只在标准输出。客户端复用当前认证会话，不额外登录；服务器请求体 180 秒限制不变。
...
- 大 PDF 或低速链路上传仍在发送时，API 可能在 multipart 请求体 180 秒总时限处主动返回 408，而客户端因仍在写正文未及时读取该响应。先请系统项目按同一时间窗核查 API/代理日志、任务 `receivedFiles` 和临时文件；若确认 API 主动 408、目标文件未登记且无临时文件，不能写成 Nginx 超时、隐性上传成功或本地文件缺失。停止当前请求后继续沿用同一任务，先回读服务端水位，只重新打开并补传未登记文件；不得新建任务、整案重传或自动循环重试。
