...
默认使用整文件上传。显式 `--upload-mode resumable` 的能力探测、上传计划、分块及原任务恢复见 [分块上传](chunk-upload.md)；服务端尚未发布时不启用新写入协议，既有补录路径不变。

正文取回批次互斥的 `USER_EXPORT_RECALL_BATCH_BUSY` / `EXPORT_RECALL_BATCH_BUSY` 保留为结构化等待原因，归入限流等待；有标准数字 Retry-After 才记录下一次重查时间，缺失保持未知。等待秒数不是后台完成时限，不因等待退出重复创建取回批次，后台 SUCCEEDED 也不代替正文哈希比对。

飞牛是案卷正文的唯一长期存储库。Meifu 只在飞牛断线时临时接替写入或在需要浏览/下载时中转；飞牛恢复后由系统修复临时接替任务，不把 Meifu 描述为备份。

本流程只适用于现有登记系统 V2，所有业务请求位于 `/api/v2`。来源网页认证和浏览器采集是独立上游；当前授权允许自行登录时沿用 LocalVault 标准输入入口。不向 V2 接口传递来源 Cookie、RWID、截图、URL 或 HTML。
...
5. 两个及以上新案卷使用 `upload-batch`，两个及以上既有案卷缺失文件补录使用 `supplement-batch`：先离线预检全部显式项目编号，再登录和读取会话各一次、就绪检查一次，在同一客户端 Cookie 容器中依次处理。跨大队批量必须是 ADMIN；同一大队的 BRIGADE 批量仍逐案核对 `brigadeCode`。普通查询和控制请求的起始间隔不少于 1.05 秒，PDF 正文走独立上传额度；不得用外层循环逐案启动单案命令。
...
- 只有服务端任务为 CREATED、UPLOADING 或 MANIFEST_RECEIVED，且包哈希、`projectNo`、文件投影、清单绑定、大队、目标和身份摘要完全一致时，才能续传。服务端 GET 必须返回 `receivedFiles`，逐项核对上传相对路径、SHA-256 和大小；manifest 已提交后还可用其中的 `clientRef` 辅助对账，服务端若返回 MIME 再核对 MIME。服务端精确多出的已接收文件可以推进本地 `uploadedFileRefs`；本地声称已传但服务端缺失、服务端出现额外路径或任一属性不一致时立即停止。不接受 `files`/`uploadedFiles` 旧兼容字段，接收清单缺失时不得推测文件已上传。
- CREATED/UPLOADING 只上传 `uploadedFileRefs` 之外的文件，全部引用齐全后才提交 manifest。MANIFEST_RECEIVED 的接收清单必须完整等于本地投影，才能直接 finalize；接收清单缺失或不完整均停止，不重复上传 PDF 或 manifest。
- 单文件上传默认总期限 630 秒、连续无进展 60 秒，支持 Ctrl+C 取消。进度输出到标准错误流，最多每 2 秒更新，包含文件名、已确认接收数量、发送字节和耗时；发送字节不代表服务端已接收。机器可读结果仍只在标准输出。客户端复用当前认证会话，不额外登录。客户端期限用于配合系统侧 API 600 秒、代理 630 秒方案；系统侧部署与实际生效须由开发任务另行回读，本地 Skill 更新不证明上游时限已改变。
- 超时、断线、取消、408、429 或接收回执校验失败后，停止正文发送并立即用同一任务回读接收清单。对账成功只更新精确已接收引用并保存断点，本案仍停止；下一次命令先对账再续传，不自动重发正文、不创建替代任务。
...
本地状态报告将已确认进度与本次请求结果分开：

- 同一 V6 任务投影、清单和身份完整对账后，部分文件已接收显示 `PARTIAL_UPLOAD`；尚无已接收文件显示 `UPLOAD_INTERRUPTED`；文件齐备但尚未正式建档显示 `UPLOAD_PENDING_FINALIZE`。这些状态均计入 `uploadPending`，不算批次完成，不创建替代任务。接收数量是已保存的服务端对账断点，不冒称实时服务端状态。
- 已建档后的限流、等待超时、网络等待分别保留为 `RATE_LIMITED`、`WAIT_TIMEOUT`、`NETWORK_WAIT`；一般落盘等待/取回缓存配额不足保留 `PENDING` 与明确原因。普通等待不写成正文损坏或正式导入失败；真实哈希、归属、权限错误仍需核查。
- 有效数值型 Retry-After 记录等待秒数、观测时间与下次可重试时间；缺失或无效则明确未知，不猜秒数、不据此循环请求或重新登录。现有有界重试策略不扩大。
- `ledger status/report` 是只读投影，可展示缺失文件和等待原因，但不会自动重试、改水位或修复历史人工复核状态。历史误分类须由执行任务在当前授权下核验同一案卷后更新，不能批量手改旧水位或 V6 状态。
...
- 429：先区分认证、普通 API 和 PDF 上传阶段。普通请求只有在收到数值型 `Retry-After` 且单次、累计等待均不超过 60 秒时，才按该秒数加短随机抖动有限重试；响应头缺失、等待超过 60 秒或有限重试耗尽时明确报告等待秒数并保留断点。流式 PDF 收到 429 不在同一打开文件上自动重放，下一次命令重新打开文件并先以服务端 `receivedFiles` 对账。多案继续使用一个批量认证会话，不得并发登录、长时间自动睡眠或无界重试。
- 大 PDF 或低速链路上传仍在发送时，API 可能达到当前生效的 multipart 请求体总时限而主动返回 408，客户端因仍在写正文未及时读取该响应。历史线上证据出现过约 180 秒的 API 主动 408；提高本地总期限不能消除尚未部署的上游限制。先请系统项目按同一时间窗核查 API/代理日志、实际配置、任务 `receivedFiles` 和临时文件；若确认 API 主动 408、目标文件未登记且无临时文件，不能写成 Nginx 超时、隐性上传成功或本地文件缺失。停止当前请求后继续沿用同一任务，先回读服务端水位，只重新打开并补传未登记文件；不得新建任务、整案重传或自动循环重试。
...
- 服务器统一错误回执中的 `requestId`、`timestamp`、`details` 只作为忽略的外层元数据；客户端只显示经过长度、敏感词和路径过滤的简短业务提示，不输出这些元数据或完整响应。
...
## 原系统案卷、材料缺口与续核（1.10.3）

服务器原有案卷若没有本地 V6 上传任务，使用 verify-existing；此命令核对当前正式案卷、完整清单、文件身份及内容代际，不上传、提交清单或重新 finalize。正文完成且来源材料齐全时，显式 --archive 归档本地原件与核验回执。历史导入的 packageSha256 与后来重新采集的 ZIP 摘要可不同，两者分别记载，不能修改历史导入任务或伪造上传状态来对齐。

原服务器人工维护的字段优先作为当前业务事实；如需修正用于核验的旧本地清单，应先留存原清单、差异及新摘要，禁止为通过核验回写覆盖人工字段。

正文下载遇 429 时沿用结构化等待状态及 Retry-After，立即停止该份请求，保存已成功的逐份回执；不得标为哈希失败。等待后只续核缺失文件，不重新上传或重复整卷准备。归档命令的运行日志应放工作区/核验记录，不在待移动的项目目录里保持打开文件句柄；Windows 归档失败先核实回滚，再释放本次日志句柄并重试本地归档。

content-lease.json 单独保存本次整卷保留任务，绑定项目、服务器、案卷、清单及全部文件身份。准备阶段第一次回执即落盘，超时或正文中断后读取原准备任务；未到期继续续租，到期保留 EXPIRED，按剩余文件处理，不能替换导入任务。正文全部核验后检查释放回执，再记 RELEASED；释放失败仍保留原租约断点。旧版没有本地租约回执的中断案，只在该案正文已完整核验、当前目录仍一致时读取其原准备状态并释放，不通过准备新案来绕过全局并发限制。
