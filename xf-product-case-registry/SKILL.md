...
只适配当前消防产品案卷信息登记系统 V2。飞牛是正文长期存储库；Meifu 只在飞牛断线或浏览下载需要时临时中转，不能把中转区描述成备份。不得改动网站、数据库、接口、部署或 `xf-product-case-filler`。
...
- 固定目录结构为：`原始案卷/<项目编号>` 保存收到的案卷包及原件；`工作区/<项目编号>` 保存清点、OCR、拆分、`normalized/`、清单和上传状态；`工作区/核验记录` 可选保存核验摘要（CLI 不自动写入）；`资料模板` 只保存可复用模板。不得用工作区替代原始案卷目录，也不得把原件移动、改名、覆盖或删除。
...
- `validate` 与 `upload --dry-run` 都不写网站；正式导入必须显式使用 `upload --finalize`。导入完成后用 `verify` 逐项核对案卷、检查、产品、目录版本、目录 SHA-256、飞牛状态和落盘核验时间。
- 产品可选 `repairSiteId` 必须是 UUID 或 `null`；填写非空 UUID 时必须同时填写 `maintenance: "YES"`。`compose` 会保留该字段，`validate` 和 `verify` 会按生产契约核验。
- `verify` 默认低流量只核对目录 SHA-256、飞牛状态和落盘核验时间；需要下载正文时显式增加 `--deep-content-verify`。深度核验遇服务端结构化 `RECALL_REQUIRED` 会使用当前认证会话自动发起或复用取回任务，轮询 `READY`、`PENDING`、`PROCESSING`、`OFFLINE`、`FAILED`，取回完成后再下载校验；网络异常或等待超时只报告核验未完成，不伪报 SHA-256 不一致。
- `upload --dry-run` 保持完全离线，不读取认证配置。`upload` 与 `verify` 的原有参数保持兼容，并新增可选 `--auth-config`。
- `upload-state.json` 使用封闭的 V6 字段集：保存规范化文件投影、目标、包哈希、任务进度、大队编号和不可逆身份摘要，不保存用户 ID、大队 ID、密码、Cookie、CSRF 或完整服务端响应。终结结果只白名单保存案卷 ID、服务端 finalizedAt 和计数摘要，命令最终也只输出摘要。状态文件只可位于 `工作区/<项目编号>`，用于防止误续传和误切换身份，不带 HMAC，也不是授权边界；真正的安全边界始终是服务端会话、CSRF 与大队授权。旧 V5 状态、文件投影变化或服务端任务无法对账时禁止续传，可单独运行 `verify` 只读核验。
- `upload-state.json` 的文件投影同时保存本地实测页数和字节数，并保存不可变清单绑定摘要；服务端任务为 CREATED、UPLOADING 或 MANIFEST_RECEIVED 且绑定完全一致时才可幂等重传全部文件。finalize 摘要出现冲突、跳过项或 `created=false` 时进入 `FINALIZED_WITH_CONFLICTS`，必须人工修正，不自动核验或标记 VERIFIED。
