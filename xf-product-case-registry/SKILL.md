...
- CLI 登录后必须回读会话并核对身份、认证方式、CSRF、首次改密状态和大队范围。全 8 大队批量上传或补录必须使用 ADMIN；BRIGADE 账户只能处理与 manifest `brigadeCode` 一致的本大队案卷。多案正式写入只用 `upload-batch` 或 `supplement-batch` 共用一次会话；不得用循环逐案调用单案命令造成重复登录。HTTP 429 读取 `Retry-After` 后保留断点并按提示等待，不自动长时间睡眠或反复登录。
