...
description: 执行或维护 WSL X 消息监控；隐藏 Chrome MCP 单页采集、模型语义与额度筛选、双流入账及飞书对账。用于 heartbeat、排障和优化，按项目阶段串行执行，不创建并行扫描者。
metadata:
  x-custom-skill: true
...
固定任务、调度、登录账号和飞书目标读取项目当前部署绑定；没有业务授权不改变。只用已接受的 hidden_chrome_mcp、常驻连接和受管工作页，普通结束 park，不重开已关闭的维修跟进。生产周期由登记任务运行；开发/验证按用户授权与全局规则优先在当前任务直接完成，单执行者约束资源并发而非永久任务独占。涉及共享资源先核对原执行者空闲、无在途/未知效果，以当前真实身份通过受管入口串行接续；不冒用原任务证明、不并行扫描。项目旧流程与当前已授权需求冲突时说明并修订，不重复索要同一授权；阶段变量不改变系统或工具权限。
