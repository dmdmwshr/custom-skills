...
先核对主机和业务仓 PROJECT_HANDOFF.md 当前状态，不加载历史恢复过程。本技能描述业务方法，不拥有电脑环境：浏览器安装、用户权限、代理、进程证明及重启恢复交给当地受管环境入口。当前已验证实现是 WSL MCP；迁移时部署对应环境适配并验证，不改写本技能来固定新的用户名、UID或安装路径，也不自动恢复退役后端。
...
固定任务、调度、登录账号和飞书目标读取项目当前部署绑定；没有业务授权不改变。只用已接受的 hidden_chrome_mcp、常驻连接和受管工作页，普通结束 park，不重开已关闭的维修跟进。开发负责维修，登记的固定任务独占实际采集；不从中枢或 CLI 再建扫描者。
