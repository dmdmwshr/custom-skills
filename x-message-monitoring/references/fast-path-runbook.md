...
- 可复制的 locator readiness 形状（仍在同一 `tab.playwright` 单候选调用内）：只以动态 `targetStatusId` 构造 `targetTimeLink = 'article[data-testid="tweet"] a[href$="/status/' + targetStatusId + '"] time'`，以状态路径精确结尾避免 ID 前缀误命中，不拼接作者、不要求当前视口可见。调用起始即设置 `deadlineAt = Date.now() + 5000` 与 `remaining() = Math.max(1, deadlineAt - Date.now())`，以下全部等待只用 `timeout: remaining()`，不得各自重置为 5 秒。先等待唯一主列内的 `targetTimeLink` attached；再令 `sectionCandidates = tab.playwright.locator('[data-testid="primaryColumn"] section[role="region"][aria-labelledby]:has(' + targetTimeLink + ')')`，`conversationLocator = sectionCandidates.last()`，并等待它 attached。同一 statusId time-link 的匹配 section 只能是包含该 DOM 链接的嵌套祖先，故 `last()` 是预批准的最小 section。再令 `topLevelTweetLocator = conversationLocator.locator('article[data-testid="tweet"]:not(article[data-testid="tweet"] article[data-testid="tweet"])')`，等待第二张非嵌套 tweet attached；随后读取一次 `topLevelCount`，并在剩余总截止时间内用一个 `Promise.all` 并发等待这批顶层卡各自至少出现一个 `a[href*="/status/"] time` attached。该批量等待只证明当前成员不再是无身份骨架，不把 quote 时间链接或任一卡指定为父帖；最终同步提取仍须排除 quoteTweet 并证明每张卡恰有一个自身规范 time-link。任一成员在总截止前未就绪统一进入 `permalink_members_not_ready`。所有等待都不读取正文，随后仅一次同步 `evaluate` 验证唯一 closest 容器和完整非嵌套顶层链。
...
- 稳定身份链必须为 2～200、ID 全唯一，目标恰好一次且索引至少为 1，`parentIndex = targetIndex - 1` 的槽位精确命中父/目标；随后只对这两张卡执行 `parseTargetDetail` 与 `parseParentFacts`。目标必须与 Latest 冻结观察的状态 ID、作者、规范 UTC、规范永久链接、可见正文或媒体标记、回复/转推/引用标志逐项一致；任何不一致使用 `permalink_final_observed_mismatch`。父/目标均须为顶层、非引用、非推广，父作者等于回复对象且父 UTC/数值 ID 早于目标。状态 ID 顺序不得使用 `BigInt`：固定用 `compareDecimalStatusIds(left,right)` 去掉多余前导零后先比较长度、等长再按 ASCII 十进制字符串比较；只有严格 `parent < target` 才通过，否则返回 `permalink_parent_time_order_untrusted`。父目标卡自身的稳定身份或目标详情不可解析时使用 `permalink_target_or_parent_detail_invalid`。宽链携带完整 ID 链和显式索引；精确二卡链也可携带索引。任一不可信条件均失败关闭。
- 父帖有可见正文时一并保留其原文和规范永久链接供后续 AI 判断；不得用引用卡、祖先卡或媒体占位代替父正文。`parseParentFacts` 必须在同一次页面 `evaluate` 内返回 `parentReadable`，且只有非空可见正文、非媒体、非引用、非推广时才为 true；页面外禁止引用页面内的 `marker` 或其他局部变量。`parentReadable=true` 才把父帖正文与永久链接成对写入候选；false 时完全省略这两个可选键，并把回复固定为 `ai_related=null` 的已处理/已抑制路径，不能创建普通投递。父帖稳定身份可信但正文缺失或仅媒体时，不伪造父正文、不把它当作全链身份失败。第一处无法证明唯一容器、父目标相邻或父目标稳定身份时，立即终止整条回复流；不尝试第二套 selector。
