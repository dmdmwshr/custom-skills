...
- 任务间发送或回复消息时保持目的任务现有模型、思考及服务档位，省略model/thinking/service_tier覆盖；不擅自开启Fast/加速，也不为赶预算调整这些设置。只有当前用户明确要求才修改。
...
1. 本次health无有效锁、无待finish且静态准备完成后，在functions内存初始化本轮控制对象，清除已经收口的旧引用。`heartbeat-acquire --stream-protocol independent`每轮只执行一次；手动验收另加原`--trigger-kind manual_validation`。
2. 领取调用必须放在functions.exec中：先将工具返回值赋给本次局部变量，紧接着`store`原始工具结果，之后才解析或输出摘要。完整结果必须为退出码0、schema_version=XMonitorHeartbeatLeaseV1、非空lease、可解析expires_at和已冻结expected_accounts；把完整机器回执保存为本轮唯一控制对象。只输出绑定是否成功、到期时刻和账号数，不输出lease。命令尚未结束时保存本次session_id与已收输出，仅续等该进程并在每次返回时先保存；禁止重新领取来取得第二份结果。
3. 每次预检、失败登记、publish-pending、sync-receipts和heartbeat-finish均从本轮保存对象取lease，程序序列化后经既有标准输入提交；不使用手抄常量、旧局部变量、检查点或历史结果。CUA的xMonLease只从该本轮回执初始化，首次使用前核对与functions原值严格相等，仅返回布尔值。本轮两侧都要保留至同lease两阶段收口及清理结束，才清空动态控制状态。
4. 领取解析或绑定失败时不开浏览器、不预检/采集/发布；只检查本次已保留的原始结果与本次进程，不能重做acquire。若同次结果可确定有效本轮lease，用它按原入口失败收口；若当前内存确实缺失且health仍持锁，报告“周期仍运行、等待既有TTL过期，最终状态未确认”。不从历史恢复或猜旧值、不续租/强行解锁；下一原定时轮仍按现有过期接管流程核验。TTL到期本身不能冒充finish回执。

这些步骤只固定原工具和functions/CUA内存的保存顺序，不增加客户端动作、凭证文件、进程通道或网络桥接。
...
在表中第2项本轮publish-pending预检明确成功之前，不调用draftStream、rawStream或context-plan，也不随page提前组装草稿。后续草稿由固定驱动实时生成后原样提交，不能改写collectedAt。发生时间拒绝时，清理前只从本次现存对象回传草稿collectedAt和本轮preflight_at及原样生成布尔值；旧事实不能补时重投。

## 浏览器：固定源加载，不生成新驱动

下列浏览器创建/预导航步骤属于直接原生扩展分支；WSL `hidden_chrome_cli` 按 [WSL接口](wsl-migration.md) 覆盖：标签仅由adapter.newTab创建，cycle保持createCycle原值，直接调用固定page。不能将不透明外部handle赋给cycle.ownTab，也不能预设lastUrl或导航计数；内部标签绑定与首次导航由原driver完成。业务处理及失败收口规则共用。
...
3.0.24在作者正文innerText为空时，仅接受同次DOM中唯一可见、已加载且currentSrc/src一致的X标准表情SVG：域名限abs.twimg.com与abs-0.twimg.com，alt为单个Unicode表情（可带FE0F）且与文件名码点相同。无查询/片段/用户信息/非默认端口或srcset；正文树最多32节点，仅含可见DIV/SPAN、空白及这一张图片。未知节点、隐藏/多图片、未加载、任意alt或不匹配资源仍失败。已有非空正文原样保留，头像/引用不计入；表情仅是已核验原文，语义仍由既有可信上文和额度契约判断，不据此推断重置承诺。

3.0.26在checkVisibility缺失时使用既有getComputedStyle与布局证据，至documentElement最多32层检查显示、可见性、透明度与隐藏内容；接口缺失、祖先链未到达文档根、未知样式或隐藏/透明仍拒绝。原生接口存在时保留原路径；加载/资源/码点/单图片/正文树和所有权要求不变，不把API未提供写成实际CSS隐藏。
...
- `scan-analysis`：锁前按[分析契约](reply-reset-contract.md)加载纯字段模块xMonAnalysisFields；`nodeRepl.write(await xMonStdin.sendKept("scan-analysis",{lease:xMonLease,payload:{collected:xMonStdin.kept(xMonLease,"analysis-plan").payload,analyses:xMonAnalysisFields.validateAnalyses(xMonAnalyses,"reply")}}))`，主帖改用字面量"main"。它只在发送前核对字段、返回原对象，语义由模型独立判断；回复ai_relevance用模块aiRelevance组装，不能误用reset_analysis.related。程序转换raw字段、父帖上下文、索引及指纹，形成严格XMonitorScanInputV3并调用scan-stream；模型不生成映射代码。
