...
成员等待超时不等于没有回复关系。核对同次 requested_target、等待阶段、结构元数据和 failureEvidence；页面可能暂未完成加载，或父帖被虚拟化到视口上方。已安装恢复版本在原期限内按实测结构向上补查，已知未就绪最多一次在原页重新导航精确请求URL，合计30秒；不能只reload实际停留的上一条状态。请求返回后仍校验真实URL/目标身份，不以已写入的lastUrl代替页面事实；即使下方回复已满足成员数量，提取后的上文缺失仍须检查。身份冲突和控制unknown不得套用此恢复。用户确认人工操作后立即停止自动化，不刷新、点击或重试旧页；先核对租约、在途和旧runtime释放，待人工结束再新鲜建立runtime/adapter并核验登记页身份。人工操作后的页面变化不能反推人工之前的错误原因。具体页面根因不能仅由事后正常截图推断，恢复验证和原故障证据分开记录。
