...
驱动 2.0.0、新回复上下文、ResetAnalysisV2 和 FrozenXMessageV4 以 [回复上下文与额度契约](reply-reset-contract.md) 为准。直接 Latest 回复搜索，取消回复主页前置，可信直接对象可为主帖或另一条回复；只读识别新项后必要时补三层上文，结束后一次冻结。正文不完整或关系不可信失败关闭；事实完整但语义不足才抑制为无法判断。
