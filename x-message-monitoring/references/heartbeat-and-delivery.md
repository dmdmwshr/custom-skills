...
驱动 2.0.1、新回复上下文、ResetAnalysisV2 和 FrozenXMessageV4 以 [回复上下文与额度契约](reply-reset-contract.md) 为准。直接 Latest 回复搜索，取消回复主页前置，可信直接对象可为主帖或另一条回复；只读识别新项后必要时补三层上文，结束后一次冻结。正文不完整或关系不可信失败关闭；事实完整但语义不足才抑制为无法判断。搜索和详情首屏目标缺失时，只在共享十五秒预算内刷新一次 AX 再等待，不增加失败后重试。

新回复只按 Codex 额度重置直接相关推送，AI 判断为辅助；no_time 可通知且不编造时间。新增额度无关/未评估计数，不重新解释旧的非 AI 统计。全过程异常按首次、类别/范围实质变化、满二十四小时一次、完整恢复一次去重；上轮缺完成回执由下一原心跳补报。普通通知、变化提醒和健康摘要保留现有消费者信封，新增事实由版本化本地冻结哈希保护。

下方 1.4.2、AI 回复筛选及旧提醒文字仅用于历史契约兼容，不覆盖本节。原自动化保持既有状态；本次验收和部署不擅自暂停、恢复或改动任务。
