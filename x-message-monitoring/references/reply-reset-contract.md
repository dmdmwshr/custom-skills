...
用户明确授权飞书网页可见性验收时，本轮新增通知即使只有登记回执，也可在已核验的唯一目标会话只读查看，不要求先取得 transport_accepted。用本轮新自有标签匹配消息时间和内容；不能把“未证明送达”误读为“禁止查看页面”。无法唯一确认目标会话则停止，不探索其他聊天；不通过 UI 发送、不重发，也不把人工观察自动写成机器送达回执。
