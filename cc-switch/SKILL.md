---
name: cc-switch
description: 用于管理本机 cc-switch 的 skills 仓库源、同步来源和相关 SQLite 记录，包括查询、添加、启停、改分支和删除 skill_repos；优先使用可用 CLI，CLI 异常时用随附脚本安全操作数据库。
x-custom-skill: true
...
本 skill 用于管理本机 `cc-switch`，重点是 skills 仓库源的查询、增加、启停、改分支和删除。
...
- 排查 `cc-switch` CLI 入口、settings、同步来源和仓库源状态。

## 硬规则

1. 先查询当前状态，再执行任何变更。
2. 优先使用可用的 `cc-switch` CLI；如果 CLI 入口缺失、损坏或行为不可确认，再使用本 skill 的 SQLite 脚本。
...
6. 变更后再次 `list`，必要时通过 cc-switch 同步或刷新。
